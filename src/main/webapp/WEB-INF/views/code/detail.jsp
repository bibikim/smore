...
<jsp:include page="../layout/header.jsp">
	<jsp:param value="${codeboard.cNo}번 게시판" name="title"/>
</jsp:include>
...
<div>
	...
	<h1>${codeboard.title}</h1>
	
	<div>
		<span>▷ 작성자 ${codeboard.user.name}</span>
		&nbsp;&nbsp;&nbsp;
		<span>▷ 작성일 <fmt:formatDate value="${codeboard.createDate}" pattern="yyyy. M. d HH:mm" /></span>
		&nbsp;&nbsp;&nbsp;
		<span>▷ 수정일 <fmt:formatDate value="${codeboard.modifyDate}" pattern="yyyy. M. d HH:mm" /></span>
	</div>
	
	<div>
		<span>조회수 <fmt:formatNumber value="${codeboard.hit}" pattern="#,##0" /></span>
	</div>
	...
	<div>
		<form id="frm_btn" method="post">
			<input type="hidden" name="cNo" value="${codeboard.cNo}">
			<c:if test="${loginUser.userNo == codeboard.user.userNo}">
				<input type="button" value="수정" id="btn_edit_coadboard">
				<input type="button" value="삭제" id="btn_remove_coadboard">
			</c:if>
			<input type="button" value="목록" onclick="location.href='${contextPath}/codeboard/list'">
		</form>
		<script>
			$('#btn_edit_codeboard').click(function(){
				$('#frm_btn').attr('action', '${contextPath}/codeboard/edit');
				$('#frm_btn').submit();
			});
			$('#btn_remove_codeboard').click(function(){
				if(confirm('블로그를 삭제하면 블로그에 달린 댓글을 더 이상 확인할 수 없습니다. 삭제하시겠습니까?')){
					$('#frm_btn').attr('action', '${contextPath}/codeboard/remove');
					$('#frm_btn').submit();
				}
			});
		</script> 
	...
	<script>
		...
		function fn_commentList(){
			$.ajax({
				...
				data: 'cNo=${codeboard.cNo}&page=' + $('#page').val(),
				dataType: 'json',
				success: function(resData){
					...
					*/
					...
					$.each(resData.commentList, function(i, comment){
						...
						div += '<input type="hidden" name="cNo" value="' + comment.cNo + '">';
		...
		function fn_goodCheck() { 
			$.ajax({
				...
				type: 'get',
				data: 'cNo=${codeboard.cNo}&userNo=${loginUser.userNo}',
				dataType: 'json',
				success: function(resData){
					if (resData.count == 0) {
