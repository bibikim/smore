...
<jsp:include page="../layout/header.jsp">
	<jsp:param value="게시판수정" name="title"/>
</jsp:include>
...
<script>
	...
	$(document).ready(function(){
			...
			callbacks: {
				onImageUpload: function(files){
					...
					for(let i = 0; i < files.length; i++) {
						...
						$.ajax({
							type: 'post',
							url: getContextPath() + '/codeboard/uploadImage',
							data: formData,
		...
		$('#btn_list').click(function(){
			location.href = getContextPath() + '/codeboard/list';
		});
...
<div>
	...
	<form id="frm_edit" action="${contextPath}/codeboard/modify" method="post">
	
		<input type="hidden" name="cNo" value="${codeboard.cNo}">
	
		<div>
			<label for="title">제목</label>
			<input type="text" name="title" id="title" value="${codeboard.title}">
		</div>
		
		<div>
			<label for="content">내용</label>
			<textarea name="content" id="content">${codeboard.content}</textarea>				
		</div>
